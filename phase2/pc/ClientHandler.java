...
public class ClientHandler {
                    ...
                    */
                    ...
                    if (answer.equals("1")) {
                        ...
                        if(!topic.getName().equals("")) {
	                        ...
	                        ArrayList<String> vNames = broker.getTopicVideoMap().get(topic.getName()); //λαμβάνει τη λίστα με τα video με αυτό το topic
	                        out.writeUnshared(vNames);
	                        out.flush();
                        }
    ...
    public void server() throws IOException{
    	ServerSocket serverSocketClient = new ServerSocket(broker.getPort()+1);
    	
    	while(true) {
	        try {
	        	Socket connectionClient = serverSocketClient.accept(); //δέχεται αιτήματα για connections
	        	ObjectOutputStream out = new ObjectOutputStream(connectionClient.getOutputStream());
	        	ObjectInputStream in = new ObjectInputStream(connectionClient.getInputStream());
	            ...
	            if(flag==0) {  //αν flag=1, θέλει ήδη υπάρχοντα video
	                ...
	                System.out.println(appRequesting.getChannelName());
	                ...
	                for (AppNodeInfo app : appNodeInfoList) { //για κάθε publisher
	                    ...
	                    if (!sameAppNode) { //φιλτράρισμα  για να μη λάβει ο consumer video που έχει ανεβάσει ο ίδιος
	                        ...
	                        Socket connectionToPub = new Socket(app.getIp(), 16230); //νέα σύνδεση με τον AppNode
	                ...
	                Socket connectionToPub = new Socket(appPushing.getIp(), 16230); //νέα σύνδεση με τον AppNode
	                ObjectOutputStream outToPub = new ObjectOutputStream(connectionToPub.getOutputStream());//που θα κάνει push
	                ObjectInputStream inFromPub = new ObjectInputStream(connectionToPub.getInputStream());
