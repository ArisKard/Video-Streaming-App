...
public class AppNode implements Node {
    ...
    private ObjectOutputStream out, consumerOut, serverOut;
    private ObjectInputStream in, consumerIn, serverIn;
    ...
    public void init(int i) {
        ...
        appNodeInfo = new AppNodeInfo(getChannelName(), brokerResponsible, port); //φτιάχνει ένα αντικείμενο AppNodeInfo που κρατά τις πληροφορίες για τον τρέχοντα AppNode
            ...
            while (running) {
                ...
                do {
                    ...
                    try {
                        ...
                        out.writeBoolean(ifAlreadySubscribed); //ενημέρωσε αν έχεις κάνει ήδη εγγραφή
                        out.flush();
    ...
    private void server() throws IOException, ClassNotFoundException{
        ...
        serverSocket = new ServerSocket(port); //περιμένω να λάβω αιτήματα για να κάνω push
    ...
    private void pull(Topic topic, BrokerInfo b) throws IOException, ClassNotFoundException { //pull video που έχουν ήδη ανέβει
        ...
        consumerConnection = new Socket(InetAddress.getLocalHost(), b.getPort()); //νέα σύνδεση, ως consumer, στον Broker που
        consumerOut = new ObjectOutputStream(consumerConnection.getOutputStream()); //είναι υπέθυνος για το topic που έχουμε
        consumerIn = new ObjectInputStream(consumerConnection.getInputStream()); //κάνει εγγραφή

        consumerOut.writeInt(2); //στέλνω flag=2 για να ενημερώσω ότι θέλω να λάβω video(s)
        consumerOut.flush();
    ...
    private void pull(String vName, BrokerInfo b) throws IOException, ClassNotFoundException{ //pull video που ανέβηκαν μεταγενέστερα
        ...
        consumerOut.writeInt(2);  //στέλνω flag=2 για να ενημερώσω ότι θέλω να λάβω video
        consumerOut.flush();
